...
void CSimpleGenerator::run()
{
    ...
    while(true)
    {
        ...
        /** Generate workunits for each package */
...
void CSimpleGenerator::createRegularWorkunit(PtrPackage &package, PtrHost &host)
{
    uint64_t packageId = package->getId();
    uint64_t hostBoincId = host->getBoincHostId();

    if (m_sqlLoader->getWorkunitCount(packageId, host->getId()) >= 2)
    {
    ...
    {
        /** No workunit could be generated, setting package to finishing */
        Tools::printDebugPackage(Config::DebugType::Log, packageId,
                "No more workunits available. Setting package to finishing\n");
...
void CSimpleGenerator::finishPackage(PtrPackage & package)
{
    ...
    {
        Tools::printDebugPackage(Config::DebugType::Log, packageId,
                "Timeouted package has finished all workunits. Setting state to timeout.\n", packageId);
...
{
    PtrWorkunit retryWorkunit = m_sqlLoader->getEasiestRetry(package->getId());
    ...
    if (package->getAttackMode() == Config::AttackMode::AttackDict && package->getAttackSubmode() != 0)
        retryWorkunit->setHcKeyspace(retryWorkunit->getHcKeyspace() * package->getCombSecDictSize());
