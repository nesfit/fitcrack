...
<template>
  <div>
    <v-breadcrumbs divider="/" class="pb-0"  v-if="data != null">
      <v-breadcrumbs-item>
        <router-link :to="{name: 'pcfg'}">PCFG</router-link>
      </v-breadcrumbs-item>
      <v-breadcrumbs-item>
        {{data.name}}
      </v-breadcrumbs-item>
    ...
    <v-container>
      <v-layout row wrap justify-center>
        <fc-tile title="pcfg file" :loading="data==null" class="mx-2 dictContentContainer mb-4">
            ...
            <v-list-tile class="px-2 py-1">
              <v-list-tile-content>
                <v-layout row class="width100 margintop5">
                  <v-spacer></v-spacer>
                  <a :href="$serverAddr + '/pcfg/' + data.id + '/download'" target="_blank">
          ...
          <div class="dictContent" v-if="data != null">
            <fc-textarea :readonly="true" :searchEnable="true" :url="this.$serverAddr + '/pcfg/' +  this.$route.params.id + '/data'" maxHeight="500"></fc-textarea>
          </div>
  ...
  export default {
    ...
    methods: {
      loadData: function ($state) {
        this.axios.get(this.$serverAddr + '/pcfg/' + this.$route.params.id).then((response) => {
