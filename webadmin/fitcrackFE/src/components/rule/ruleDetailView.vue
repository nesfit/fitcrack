      ...
      <fc-tile title="Rule file" :loading="data == null" class="mx-2 dictContentContainer mb-4">
        ...
        <v-row class="mx-2 mb-2">
          <v-col class="d-flex justify-center">
            <router-link :to="`/rules/edit/` + data.id">
              <v-btn color="primary">
                Edit
          ...
          <v-col class="d-flex justify-center">
            <v-btn color="primary" :href="$serverAddr + '/rule/' + data.id + '/download'" target="_blank">
              Download <v-icon right>mdi-download</v-icon>
            </v-btn>
          </v-col>
        </v-row>
        <v-divider />
        <div v-if="data != null" class="dictContent">
          <fc-textarea :readonly="true" :search-enable="true"
            :url="this.$serverAddr + '/rule/' + this.$route.params.id + '/data'" max-height="500" />
        </div>
      </fc-tile>
    </v-container>
  </div>
