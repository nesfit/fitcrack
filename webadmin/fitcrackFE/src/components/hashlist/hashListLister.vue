...
<template>
  <div class="cont">
    <v-toolbar flat>
      <h2>Hashlists</h2>
      <v-spacer></v-spacer>
      <v-btn text color="primary" :to="{ name: 'createHashlist' }">
        <v-icon left>
          mdi-book-plus
        </v-icon>
        Create new
    ...
    <div class="d-flex mx-3">
      <v-switch v-model="isTrash" label="Show discarded" class="mx-2" prepend-icon="mdi-delete-empty"
        @change="loadHashes" />
      <v-text-field v-model="search" class="mx-2 search-field" clearable prepend-icon="mdi-table-search" label="Search"
        single-line @input="loadHashes" />
      <HashTypeAutocomplete v-model="hash_type" class="mx-2" label="Filter by hash type" single-line
        prepend-icon="mdi-lock" clearable @input="loadHashes" />
    </div>
    <v-divider />
    <v-data-table ref="table" :headers="headers" :items="hashlists" :search="search" :options.sync="dataTableOptions"
      ...
      <template v-slot:item.progress="{ item }">
        <div class="d-inline-flex align-center">
          ...
          <v-progress-circular :value="item.cracked_hash_count / item.hash_count * 100" :max="100" color="success"
            size="18" rotate="270" />
        </div>
      </template>
      ...
      <template v-slot:item.actions="{ item }">
        <v-tooltip left>
          <template v-slot:activator="{ on }">
            <v-btn icon class="mx-0" v-on="on" :href="`${$serverAddr}/hashlist/${item.id}/download`">
              <v-icon>
                mdi-download
              </v-icon>
            </v-btn>
        ...
        <v-tooltip left>
          <template v-slot:activator="{ on }">
            <v-btn icon class="mx-0" v-on="on" @click="deleteOrRestore(item.id)">
              <v-icon>
